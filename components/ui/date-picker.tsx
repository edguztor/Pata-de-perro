...
export function DatePicker({ value, onChange, placeholder = "Selecciona fecha", minDate, disabled, id }: DatePickerProps) {
  ...
  return (
    <Popover.Root open={open} onOpenChange={disabled ? undefined : setOpen}>
      ...
      <Popover.Portal>
        <Popover.Content
          className="z-50 w-72 rounded-xl border border-slate-700 bg-[#0d1627] p-4 shadow-2xl animate-in fade-in-0 zoom-in-95"
          align="start"
          sideOffset={4}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    inMonth && !isSelected && !todayHighlight && !isDisabled && "text-slate-300 hover:bg-slate-700 hover:text-white",
                    todayHighlight && !isSelected && "bg-slate-700/70 text-white",
                    isSelected && "bg-[#e94560] text-white font-bold shadow-sm",
                    isDisabled && "text-slate-700 cursor-not-allowed hover:bg-transparent"
                  )}
                >
                  {format(day, "d")}
                </button>
